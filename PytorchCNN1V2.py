        ...
        for c, (input_test,targets) in enumerate (test_loader):
            ...
            with torch.no_grad():
                ...
                for x in range(size_labels):
                    chk=predict.data
                    print("type(chk)= ", type(chk))
                    print(chk)
                    print(chk[0].item())
                    print("type(predict)= ",type(predict))
                    print(predict)
                    # print("print(predict[x]) ",predict.data()[x]) # doesnt work
                    #so we can use it like: predict.data[x].item()
                    print("print(predict.data[x].item()) = ", predict.data[x].item())
