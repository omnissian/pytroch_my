...
# print("train_dataset.train_data.size() ",train_dataset.train_data.size())
print("print(len(test_dataset))", len(test_dataset))
...
class ForwardNetModel(nn.Module):
    def __init__(self, input_dim,hidden_dim, output_dim):
        ...
        self.fullyConnectedLayer2=nn.Linear(hidden_dim,hidden_dim)
        self.tanh=nn.Tanh()
        self.relu=nn.ReLU()
        self.fullyConnectedLayer3=nn.Linear(hidden_dim,output_dim)
        #fc->f(x)->fc->out
    def forward(self,x):
        ...
        out=self.fullyConnectedLayer2(out)
        out=self.relu(out)
        out=self.fullyConnectedLayer3(out)
        return out
...
#------------test----------
        if(iter%500)==0:
            ...
            for images,labels in test_loader: # in test loader!!!!
                ...
                # t_plt=images[index].view(28,28)
                # t_plt_label=str(labels[index].item())
                # fig=plt.figure(0)
                ...
                _, pred=torch.max(out_test.data, 1)
            ...
            # print("total=",total)
            print("accuarcy= ", 100.0 * correct/total,"%")
